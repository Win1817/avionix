...
public class IngestController {
    ...
    @Data @NoArgsConstructor @AllArgsConstructor
    @Schema(description = "Batch ADS-B ingest request")
    ...
    public static class ApiResponse<T> {
        ...
        public static <T> ApiResponse<T> ok(T d) {
            return ApiResponse.<T>builder()
                    .success(true).message("OK").data(d).timestamp(Instant.now()).build();
        }

        public static <T> ApiResponse<T> ok(T d, String m) {
            return ApiResponse.<T>builder()
                    .success(true).message(m).data(d).timestamp(Instant.now()).build();
        }
    }
    ...
    @Operation(summary = "Ingest ADS-B position batch (dump1090 / VRS format)",
        description = "Accepts batched ADS-B JSON. Normalizes to internal schema, writes to " +
                      "`surveillance_reports`, and publishes to `avionix.surveillance.positions` Kafka topic.")
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "ASTERIX records parsed")
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "Flight plan ingested")
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "Duplicate flight plan for today")
    public ResponseEntity<ApiResponse<Map<String, Object>>> ingestFixm(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
            .body(ApiResponse.ok(
                Map.of("id", UUID.randomUUID(), "ingestedAt", Instant.now()),
                "FIXM flight plan ingested"));
    }
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "Bulk positions ingested")
